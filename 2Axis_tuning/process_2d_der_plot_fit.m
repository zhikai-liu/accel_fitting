function process_2d_der_plot_fit(filename_h)
...
for i =1:length(f_abf)
    ...
    for j =1:length(S.poi)
        ...
        print([S.name '_period_' num2str(j) '_' orien{h} '_raw_fit.jpg'],'-r300','-djpeg');
...
% end
% print([S.name '_period_' num2str(j) '_' orien{h} '_cycle_fit_fista.jpg'],'-r300','-djpeg');
        close all;
    end
    end
end
