...
for i = 1:n
    ...
    hold on;
    %plot([1,length(period)].*trial.si.*1e-6,[-100,-100],'k--','LineWidth',2.5)
    ylabel('pA');
    ylim(pA_YLIM_range);
    xlim(XLIM_range);
...
for i = 1:n
    set(h(i).pA,'Units','normal',...
         'position',[0.1,0.1-0.25*i+0.1,XAxis_l,pA_YAxis_h],...
         'Visible','off')
...
set(h(2*n+1).scaleBar,'Units','normal',...
         'position',[0.8,0.1,scale_XAxis,scale_YAxis],...
         'Visible','off')
text(0.2,1.3,['\color{red}' num2str(round(unit_YAxis_pA,2)) 'pA'],'fontsize',40,'fontweight','bold')
...
text(1.1,0,[num2str(round(unit_XAxis_time*1000,2)) 'ms'],'fontsize',40,'fontweight','bold')
print('one_example_origin','-dsvg')
print('one_example_origin','-r300','-djpeg')
